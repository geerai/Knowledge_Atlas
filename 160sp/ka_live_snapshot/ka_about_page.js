...
(function () {
  ...
  function wireSpecLink() {
    ...
    if (!link) return;
    const forceVisible = new URLSearchParams(window.location.search).get('fnspec') === '1';
    function hasRealSession() {
      return !!(localStorage.getItem('ka_access_token') && localStorage.getItem('ka_current_user'));
    }
    function syncLinkVisibility() {
      link.style.display = hasRealSession() || forceVisible ? 'inline' : 'none';
    }
    syncLinkVisibility();
    ...
    window.addEventListener('storage', function (e) {
      if (e.key === 'ka_access_token' || e.key === 'ka_current_user') {
        syncLinkVisibility();
      }
    });
  }
