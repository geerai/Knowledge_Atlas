...
PAGES = [
    ...
    # existing surfaces.
    {
        ...
        "files": [
            ...
            ("missing", "approval UI inside <code>/160sp/ka_admin.html</code>", "The instructor admin console already hosts the queue review framework; T2.d adds a new panel that drains the checkbox queue into a decision list, renders inline DOI preview and abstract display, and lets the admin one-click-accept each candidate with an optional reason tag. All writes are routed through <code>atlas_shared.bundle_router.QuestionBundleRouter</code> so every accepted reference lands under the correct question bundle."),
        ],
    ...
    },
    {
        ...
        "files": [
            ...
            ("missing", "tighter relevance vetter", "A deeper classifier than <code>atlas_shared.classifier_system.AdaptiveClassifierSubsystem</code>&apos;s broad-relevance check. Must ship as a new module inside <code>atlas_shared</code> (pull request by DK, not by Track 2 code). DK&apos;s note explicitly flags this as required."),
